...
export function CaravanManager({ initialCaravans, initialUserPrefs }: CaravanManagerProps) {
  ...
  const [caravans, setCaravans] = useState<StoredCaravan[]>(initialCaravans);
  const [activeCaravanId, setActiveCaravanId] = useState<string | null | undefined>(initialUserPrefs?.activeCaravanId);
  ...
  const saveCaravanMutation = useMutation({
    ...
    onSuccess: (savedCaravan) => {
      const updatedCaravans = editingCaravan
            ? caravans.map(c => c.id === savedCaravan.id ? savedCaravan : c)
  ...
  const deleteCaravanMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
        const updatedCaravans = caravans.filter(c => c.id !== deletedId);
        setCaravans(updatedCaravans);
        updateAnalyticsUserProperties(updatedCaravans);
