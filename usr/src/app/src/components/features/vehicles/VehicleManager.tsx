...
export function VehicleManager({ initialVehicles, initialUserPrefs }: VehicleManagerProps) {
  ...
  const [vehicles, setVehicles] = useState<StoredVehicle[]>(initialVehicles);
  const [activeVehicleId, setActiveVehicleId] = useState<string | null | undefined>(initialUserPrefs?.activeVehicleId);
  ...
  const deleteVehicleMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
        const updatedVehicles = vehicles.filter(v => v.id !== deletedId);
        setVehicles(updatedVehicles);
        updateAnalyticsUserProperties(updatedVehicles);
                ...
                <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-3">
                  <div className="flex-grow">
                    ...
                    <div className="text-sm text-muted-foreground font-body grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-1 mt-2">
                      ...
                      <span className="flex items-center"><Weight className="w-3.5 h-3.5 mr-1.5 text-primary/70"/> Kerb: {formatDimension(vehicle.kerbWeight)}</span>
                      {vehiclePayload !== null && <span className="flex items-center"><Backpack className="w-3.5 h-3.5 mr-1.5 text-primary/70"/> Payload: {vehiclePayload.toFixed(0)}kg</span>}
