...
5.  **CRITICAL: VERIFY THE "AFTER PAYMENT" REDIRECT.**
    ...
    *   The URL in the box must match your running application's URL, followed by `/subscribe/success`. This URL is set in your `.env.local` file as `NEXT_PUBLIC_APP_URL`. For example: `https://6000-firebase-studio-1748946751962.cluster-isls3qj2gbd5qs4jkjqvhahfv6.cloudworkstations.dev/subscribe/success`
...
3.  **Start Event Forwarding:** Open a **new, separate terminal window** (do not stop your Next.js app) and run this exact command, replacing the port if necessary:
    ```bash
    # Use the port that matches your NEXT_PUBLIC_APP_URL (e.g., 6000)
    stripe listen --forward-to localhost:6000/api/stripe-webhook
    ```
